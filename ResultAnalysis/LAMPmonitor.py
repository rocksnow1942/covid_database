...
class Analyzer():
    ...
    def initialize(self):
        ...
        files = list(filter(lambda f: f.endswith(
            '.csv') and os.path.isfile(f), files))
    ...
    def sync(self):
        ...
        if plate['companion']:
            ...
            if not companionPlate:
                ...
                return
            # step != read means this plate havn't updated raw data.
            if companionPlate['step'] != 'read':
                self.debug(
                    f'companion - {plate["companion"]} not at read step.')
                return
    ...
    def writeOnePlateToCSV(self, plate):
        ...
        file = os.path.join(
            TABLE_OUTPUT_FOLDER, f'{datetime.now().strftime("%Y%m%d %H%M")} {pType} {id}.csv')
        col = ['']+[str(i) for i in range(1, 13)]
        linesRatio = [
            ','*12, f'{id} {pType} Normalized RFU Ratio'+','*12, ','.join(col)]
    ...
    def parseResult(self, p1, p2):
        ...
        if (layout.startswith('Sample88_2NTC_3PTC_3IAB') or
            ...
            NTCs = {n: [i[w]['ratio'] for w in ['A12', 'B12']]
                    for (n, i) in [('N7', Nwells), ('RP4', RPwells)]}
            results = []
    ...
    def getPlate(self, id, file=None):
        ...
        try:
            res = requests.get(
                self.url('/plates/?page=0&perpage=1'), json={'plateId': id})
            if res.status_code == 200:
                if res.json():
                    plate = res.json()[0]
                    return plate
                    ...
                    if file:
                        self.fileHistory[file].update(
                            error=f'plate ID {id} not in database')
                    return None
            else:
                if file:
                    self.fileHistory[file].update(
                        error=f'server error: {res.status_code}')
                self.error(
                    f'sync->getPlate server response {res.status_code}:{res.json()}')
                return None
        except Exception as e:
            if file:
                self.fileHistory[file].update(
                    error='server error: can not request.')
    ...
    def uploadPlate(self, plate,handler):
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                self.error(
                    f'sync:uploadPlate server response {res.status_code},{res.json()}')
        except Exception as e:
